...
**What's Working:**
- New `ClaudeSessionsCLI` binary with `list` (one-shot JSON / NDJSON stream / Spectre.Console live table showing root + current cwd) and `focus <prefix>` verbs
- New MAUI Sessions tab (sibling to Display Manager via AppShell flyout) showing per-session cards: session title (Claude Code's generated `ai-title`, read from the transcript by `TranscriptTitleReader`; hidden when the session has none), state badge, root/start cwd, current cwd, last-activity, a Copy-path button (copies the transcript `.jsonl` path to the clipboard; hidden for transcript-less sessions), Focus button, plus optional Refresh button
