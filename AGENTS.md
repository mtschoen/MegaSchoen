...
- **Fresh and resumed sessions** - a process attaches even when the 30s start-time match misses, including multiple sessions sharing one cwd (resolved by causal start-order), not just single-process cwds.
...
- `ActiveSessionEnumerator.AssignProcesses` attributes a live process to each session (driving the window/title and `SshClientPort`) in three passes, each attaching ONLY on an unambiguous pairing (focusing the wrong terminal is worse than a disabled Focus button): (1) **confident start-time match** - a process whose StartTime is within 30s of the transcript's creation; (2) **causal forced-move** - a process can only have written a transcript created at or after the process started, so a session whose creation time has exactly one causally-plausible free process (StartTime <= creation) takes it even past the 30s tolerance, iterating so one assignment can force a peer's; (3) **last-one-standing** - when exactly one session and one process remain unattributed, elimination pairs them regardless of causality (covers a resumed session whose old transcript predates its restarted process). The 30s match misses in rig-verified ways: on Linux, .NET's `File.GetCreationTimeUtc` tracks mtime, so `TranscriptCreationTime` reads the statx birth time instead (`Claude.Core/Linux/LinuxFileBirthTime.cs`; falls back to mtime only on filesystems without btime - issue #37); on Windows, a freshly started claude writes no transcript until the first prompt, so either the freshest *old* transcript surfaces (resume - found via Rider) or the new transcript is created >30s after the process started (slow first prompt - found 2026-06-20 with two schoen-lab sessions sharing a cwd, both Focus-grayed). A shared cwd is no longer always best-effort: causal pass 2 attaches both sessions when their process start order is unambiguous; it still refuses when two-or-more processes are equally plausible (or none are) for a session.
