...
- **Remote / SSH Claude sessions.** v1 only resolves Claude sessions running locally in cmd.exe on this machine. Sessions running on remote hosts over SSH won't be reachable by the local-only `ProcessResolver`. The remote hook bridge would need to write state to a network-reachable location (shared file on a mounted volume, or a small service) and the resolver would need to probe remote hosts. See `C:\Users\mtsch\llamalab\tools\cli_dashboard.py` for an existing pattern that probes Claude / related processes over the network — adopt a similar approach when implementing.
